...
class GATv2Layer(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, heads: int = 4, dropout: float = 0.1) -> None:
        ...
        self.attn = nn.Parameter(torch.randn(heads, out_dim) * 0.01)
    ...
    def forward(self, x: torch.Tensor, adj: torch.Tensor | None) -> torch.Tensor:
        if x.dim() == 2:
            x = x.unsqueeze(1)
        b, n, _ = x.shape
        src = self.lin_src(x).view(b, n, self.heads, self.out_dim)
        dst = self.lin_dst(x).view(b, n, self.heads, self.out_dim)
        if n == 1:
            return self.dropout(torch.relu(src.mean(dim=2))).squeeze(1)
        combined = src.unsqueeze(2) + dst.unsqueeze(1)
        e = F.leaky_relu((combined * self.attn).sum(dim=-1), 0.2)
        ...
        out = torch.einsum("bijn,bjnh->binh", alpha, src)
        pooled = out.mean(dim=1)
        return pooled.squeeze(1) if pooled.size(1) == 1 else pooled


class GATv2Module(BaseGraphModule):
    def __init__(self, in_dim: int, hidden_dim: int, num_layers: int = 2, dropout: float = 0.1, heads: int = 4) -> None:
        ...
        self.layers = nn.ModuleList([GATv2Layer(dims[i], hidden_dim, heads, dropout) for i in range(num_layers)])
